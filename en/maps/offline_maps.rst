...
Instead of downloading the map tiles of a specific region, it is also possible to only download the map tiles over which a route is going. For that open the route details screen of a route, scroll down to the section 'Map tiles' and tap 'Download'.
